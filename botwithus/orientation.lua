...
local function check(raw)
  if type(raw) ~= "number" or raw ~= math.floor(raw) or raw < 0 or raw >= M.FULL_TURN then
    error(("orientation must be 0..%d or %d: %s"):format(M.FULL_TURN - 1, M.UNKNOWN_RAW,
                                                        tostring(raw)), 3)
  end
end

-- Decode the producer's zero-extended u16: WIRE_UNKNOWN -> UNKNOWN_RAW, anything else is
-- the angle. A value the producer promises never to publish raises rather than wrapping.
function M.from_wire(wire_value)
